module.exports = function ({ app, io, redisClient, db, path, fs }) {
    // Impostor Sözlük Yapısı
    ...
    function kelimeleriYukle() {
        try {
            if (fs.existsSync(dictionaryPath)) {
                ...
                console.log("📖 [Impostor] Sözlük hafızaya alındı! Kategoriler:", Object.keys(dictionary).join(', '));
            } else {
                console.log("⚠️ Uyarı: Impostor dictionary.json bulunamadı!");
            }
        } catch (err) {
